...
import { ESIM_COUNTRIES, resolveCountryCode } from '@/lib/esim-countries'
        ...
        <div className='space-y-4 px-6 py-5'>
          ...
          >
            {going ? (
              ...
              <>
                <ArrowRight className='size-4' /> View Top-Up Plans
              </>
            )}
          </motion.button>
        </div>
...
/* ── Dropdown ────────────────────────────────────────────────── */
// z-[200] so it always floats above the feature bar below the hero
function Dropdown({
  label,
  value,
  options,
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            // ✅ z-[200] — above feature bar (z-auto) and everything else on the page
            className='absolute left-0 top-full z-[200] mt-2 w-full min-w-[160px] overflow-hidden rounded-xl border border-border bg-card shadow-2xl'
          >
...
function DestinationInput({
  ...
  // Memoised filter — only rerenders when value changes, not on every keystroke of
  // parent state. Capped at 50 for DOM perf.
  const filtered = value.trim()
    ? ESIM_COUNTRIES.filter((o) =>
        o.label.toLowerCase().includes(value.toLowerCase()),
      ).slice(0, 50)
    : ESIM_COUNTRIES.slice(0, 20) // show only 20 when empty — enough for quick pick
      ...
      <AnimatePresence>
        {open && filtered.length > 0 && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -4, scale: 0.98 }}
            transition={{ duration: 0.15 }}
            // ✅ z-[200] — same level as other dropdowns, above feature bar
            className='absolute left-0 top-full z-[200] mt-2 w-full min-w-[240px] overflow-hidden rounded-xl border border-border bg-card shadow-2xl'
          >
            <div className='max-h-72 overflow-y-auto py-1'>
              {filtered.map((o) => (
                <button
                  ...
                  type='button'
                  onClick={() => {
                    // ✅ Only updates the text — does NOT navigate
                    // Navigation only happens when Search button is pressed
                    onChange(o.label)
                    setOpen(false)
                  }}
                  className={cn(
...
function EsimHeroSearch() {
  ...
  // ✅ useCallback so buildQuery reference is stable — not rebuilt every render
  const buildQuery = useCallback(
    (dest: string) => {
      ...
      if (
        ...
        q.set('tab', 'global')
      } else if (['europe', 'eu'].some((k) => d === k || d.startsWith(k))) {
        q.set('tab', 'global')
        q.set('regions', 'Europe')
      } else {
        q.set('tab', 'local')
        // ✅ resolveCountryCode now uses exact/starts-with — no wrong country matches
        const code = resolveCountryCode(d)
        if (code && code !== 'global') q.set('country', code)
      }
  ...
  // ✅ Only called when Search button is pressed — NOT on dropdown item click
  ...
  return (
    <div className='mx-auto mt-8 w-full max-w-3xl'>
      {/*
        ✅ overflow-visible — critical fix for dropdowns being clipped.
        The card background + border is achieved via the inner p-3 div.
        overflow-hidden was never on this element but the parent section
        had overflow-hidden which we isolate by making the search sit in
        a relative container with a high z-index.
      */}
      <div className='relative z-10 rounded-2xl border border-border bg-card p-3'>
        <div className='flex flex-col gap-2 sm:flex-row sm:items-stretch'>
          {/* Destination — grows to fill */}
          ...
          {/* Search — the ONLY thing that navigates */}
          <motion.button
...
export function EsimLandingClient() {
  ...
  return (
    <div className='min-h-screen bg-background'>
      ...
      {/*
        ✅ overflow-hidden removed from hero section — it was clipping dropdowns.
        The grid texture and blur orb are purely visual and don't need clipping.
        On mobile the blur orb is large but pointer-events-none so no impact.
      */}
      <section className='relative bg-background'>
        ...
        <div className='relative page-container py-20 sm:py-28 lg:py-36'>
          ...
          >
            <h1 className='font-heading text-4xl capitalize font-black leading-none tracking-tight text-foreground sm:text-5xl lg:text-6xl xl:text-7xl'>
              stay connected
            </h1>
            <h1 className='font-heading capitalize text-4xl font-black leading-none tracking-tight text-primary drop-shadow-[0_0_40px_hsl(var(--primary)/0.5)] sm:text-5xl lg:text-6xl xl:text-7xl'>
              anywhere
            </h1>
          </motion.div>
      ...
      {/*
        ✅ Feature bar has no z-index set — sits at z-auto (below z-[200] dropdowns).
        No changes needed here, just confirming it doesn't interfere.
      */}
