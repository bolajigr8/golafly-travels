...
export function useEsimFilters() {
  ...
  // ✅ Track whether this is the first render — skip pushing q to URL on mount
  ...
  // ✅ Only push search to URL after mount, and only when value actually changed
  useEffect(() => {
  ...
  // ✅ Sync local input when URL q param changes externally (back/forward nav)
  useEffect(() => {
    const urlQ = params.get('q') ?? ''
  ...
  const backendFilters: EsimPackageFilters = useMemo(() => {
    console.log('backendFilters recomputing, page=', page)
