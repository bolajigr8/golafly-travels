...
import { EsimModifySearch } from '@/components/esim/landing/esim-modify-search'
import { ESIM_COUNTRIES, type EsimCountry } from '@/lib/esim-countries'
...
const DEFAULT_CHIP: EsimCountry = {
  label: 'Nigeria',
  code: 'NG',
  type: 'country',
}
...
export function EsimClient() {
  ...
  const activeChip: EsimCountry =
    ESIM_COUNTRIES.find((c) => c.code === countryCode) ?? DEFAULT_CHIP
  ...
  const isFetching = activeTab === 'local' ? localFetching : globalFetching
  ...
  console.log('--- RENDER ---', {
    ...
    isLoading,
    isFetching,
    rawPackagesCount: regionFiltered.length,
    paginationFromAPI:
      activeTab === 'global'
        ? globalData?.data?.pagination
  ...
  console.log(
    'RAW globalData:',
    JSON.stringify(globalData?.data?.pagination),
    'data length:',
  ...
  return (
    <div className='min-h-screen bg-background'>
      <div className='page-container py-6 sm:py-8'>
        ...
        {!isLoading && !isFetching && !hasError && filtered.length > 0 && (
          <motion.div
