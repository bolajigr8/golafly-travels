...
export function SeatsClient() {
  ...
  const summary = offer
    ? {
        ...
        extras: ancillaryLineItems(ancillaries, offer.slices.length, offer.passengers.length),
      }
    : undefined;

  /* Build the leg switcher from the real offer slices */
  const t = (iso: string) => new Date(iso).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit", hour12: false });
  const dd = (iso: string) => new Date(iso).toLocaleDateString("en-GB", { weekday: "short", day: "numeric", month: "short" });
  const LEGS = offer
    ? offer.slices.map((sl, i) => ({
        id: i === 0 ? "out" : "ret",
        label: `${sl.origin.iata_code} → ${sl.destination.iata_code}`,
        date: `${t(sl.segments[0].departing_at)}→${t(sl.segments[sl.segments.length - 1].arriving_at)}·${dd(sl.departure_date)}`,
      }))
    : [{ id: "out", label: "— → —", date: "" }];
